...
public class CarDAO extends BasicDAO implements ICarDAO {
	...
	public boolean registrationNoExists(String regNo) {
	    String jpql = "SELECT COUNT(c) FROM Car c WHERE c.registrationNo = :regNo";
	...
	public List<Car> findAvailableCars(CarBranch branch, String carType, Date startDate, Date endDate) {
	    ...
	    // Filter by reservation dates
	    if (startDate != null && endDate != null) {
	        hql.append(" AND NOT EXISTS (")
	           .append("  SELECT r FROM Reservation r")
	           .append("  WHERE r.car = c")
	           .append("  AND r.startDate <= :endDate")
	           .append("  AND r.endDate >= :startDate")
	           .append(")");
